            ...
            <div className={'w-full grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4'}>
                {
                    diskInfo && diskInfo?.data?.map((each, index) => (
                        <Link href={`/main?path=${each.mountPoint}`} key={index} className={'shadow hover:bg-blue-100 hover:shadow-none transition-all duration-200 select-none pl-1 py-2 pr-2 gap-2 rounded-xs flex justify-center items-center'}>
                            <div>
